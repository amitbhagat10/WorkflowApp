...
export default function PublicQuotePage() {
  ...
  async function loadQuote() {
    ...
    if (!token || token === "undefined" || token === "null") {
      setMessage("Invalid quote link. The quote token is missing.");
      setLoading(false);
      return;
    }
    ...
    if (!data || data.length === 0) {
      setMessage("Quote not found. Please check the quote link.");
      setLoading(false);
      return;
    }
  ...
  if (message) {
    return (
      <div className="mx-auto max-w-3xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-3 text-xs text-gray-500">Token: {token}</p>
      </div>
    );
  }
  ...
  if (!quote) {
    return (
      <div className="mx-auto max-w-3xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <h1 className="text-2xl font-bold">Quote</h1>
        <p className="mt-3 text-sm text-gray-500">Quote not available.</p>
      </div>
    );
  }

...
export default function QuoteDetailPage() {
  ...
  function getPublicQuoteLink() {
    if (!quote?.public_token) return "";
    return `${window.location.origin}/public/quotes/${quote.public_token}`;
  }

  function quoteMessage() {
    if (!quote) return "";

    const link = getPublicQuoteLink();
...
Please reply if you would like to approve this quote.`;
  ...
  async function sendEmail() {
    ...
    if (!quote.public_token) {
      setMessage("Public quote link is missing. Please run the public_token SQL first.");
      return;
    }
  ...
  async function sendWhatsApp() {
    ...
    const url = `https://wa.me/${digits}?text=${encodeURIComponent(quoteMessage())}`;
  ...
  async function copyQuoteLink() {
    ...
    if (!link) {
      setMessage("Quote link is not available yet. Please run the public_token SQL first.");
      return;
    }
  ...
  return (
    <div className="invoice-page">
      <div className="no-print mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-gray-500">
            Send the client a quote link, print the quote, or convert it into a job.
          </p>
        </div>
      ...
      <div className="no-print mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <button onClick={() => updateStatus("approved")} className="btn-secondary">
          Mark Approved
        </button>

        <button onClick={() => updateStatus("declined")} className="btn-secondary">
          Mark Declined
        </button>
      ...
      <div className="no-print mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <button onClick={copyQuoteLink} className="btn-secondary">
          Copy Quote Link
        </button>

        <button onClick={openPublicQuote} className="btn-secondary">
          Open Public Quote
        </button>
      </div>
      ...
      <div className="invoice-card card mx-auto max-w-4xl bg-white">
          ...
          <div className="flex flex-col justify-between gap-6 md:flex-row">
            ...
            <div className="text-left md:text-right">
              ...
              <div className="mt-4 text-sm text-gray-600">
                ...
                <p>
                  <strong>Valid Until:</strong> {quote.valid_until || "Not specified"}
                </p>

                <p>
                  <strong>Status:</strong>{" "}
                  <span className="uppercase">{quote.status.replace("_", " ")}</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200">
          <table className="w-full border-collapse text-left text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="border-b border-gray-200 px-4 py-3">Estimate Item</th>
        ...
        <div className="mt-10 border-t border-gray-200 pt-6 text-center text-sm text-gray-500">
          <p className="font-semibold text-gray-700">
            Thank you for considering HandyFlow Services.
          </p>
